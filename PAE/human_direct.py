...
def transfrom_ani(model_data, joint_len, frames):
    ret = model_data.reshape(model_data.shape[0], joint_len, frames)
    ret = ret.swapaxes(1,2)
    ret = ret.reshape(-1, joint_len)
    return ret
...
def truncate(velocity):
    seq_len = velocity.shape[0]
    split_indices = np.arange(frames, seq_len, frames) # Joe: like [121,242] if seq len is 319, timewindow is 121
    gather = np.split(np.arange(seq_len), split_indices) # Joe: split array into subarrays
    gather = np.array(gather[:-1]) # Joe: exclude the last subarray since it can be short than timewindow
    batch = velocity[gather.flatten()] # Joe: get all the velocities. shape sequence length*timewindow x joints
    return batch, gather

def input_no_window(velocity):
    batch, gather = truncate(velocity)
    print("batch shape1 ", batch.shape)
    batch = batch.reshape(gather.shape[0], gather.shape[1], -1)
    batch = batch.swapaxes(1,2)
    batch = batch.reshape(gather.shape[0], batch.shape[1]*batch.shape[2]) # Joe: shape sequence length x joints*timewindow. xxxxxyyyyyzzzz. will be transformed to [xxxx],[yyyy],[zzzz] in model.
...
if __name__ == '__main__':
    root_path = "/home/pan/Downloads/DeepLearning/Dataset"
    ...
    v_filtered, kinematic_chain, colors = utility.remove_hands(v_filtered)
    position = ReadBinary(root_path+"/Position.bin", shape[0], shape[1])
    position, _, _ = utility.remove_hands(position)
    ...
    joints = 25 #joints of the character skeleton
    ...
    # prepare input data
    seq_one_v = v_filtered[np.where(sequences == (5))[0], :]
    seq_one_p = position[np.where(sequences == (5))[0], :]
    seq_all = input_no_window(seq_one_v) # Joe: get continuous timewindow sequences from current data. 
    seq_viz = transfrom_ani(seq_all, input_channels, frames)
    
    seq_one_v_truncated, _ = truncate(seq_one_v)

    print("seq_one_v_truncated ", seq_one_v_truncated.shape)
    print("seq_viz ", seq_viz.shape)
    print("Is velocity same? ", np.array_equal(seq_viz, seq_one_v_truncated))

    # input animation
    epoch = 40
    plot_animation(seq_one_p[0], seq_viz, f"input_e{epoch}_direct", True)
    
    # build network model
  
    network = torch.load(f"/home/pan/GenerativeMotion_Joe/AI4Animation-master/AI4Animation/SIGGRAPH_2022/PyTorch/PAE/Training/{epoch}_8Channels.pt")
    network.cuda()
    network.eval()
    ...
    y_Pred_viz = transfrom_ani(yPred.cpu().detach().numpy(), input_channels, frames)
    # # output animation
    plot_animation(seq_one_p[0], y_Pred_viz, f'output_e{epoch}_direct', True)